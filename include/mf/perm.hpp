...
namespace mf {
template <typename T>
constexpr std::enable_if_t<std::is_integral_v<T>, bool> is_pow_of_2(T x) {
  return x && !(x & (x - T(1)));
}
template <typename T>
constexpr std::enable_if_t<std::is_integral_v<T>, T> log2(T x) {
...
public:
  ...
  constexpr T size() const noexcept { return table_size; }
...
private:
  ...
  public:
    ...
    constexpr container(container &&other) noexcept
        : items(other.items), len(other.len), cap(other.cap) {
      other.items = nullptr;
      other.len = 0;
      other.cap = 0;
    }
    ...
    constexpr IdxType size() const noexcept { return len; }

    constexpr ItemType &operator[](IdxType i) noexcept { return items[i]; }
    ...
    public:
      ...
      constexpr const ItemType &operator*() const noexcept { return *it; }

    private:
      ItemType *it;
    };
    constexpr iterator begin() const noexcept { return iterator(items); }
    constexpr iterator end() const noexcept { return iterator(items + len); }
  ...
  constexpr container<T, container<T, T>>
  oneline2cyclic(const T (&oneline)[N]) {
