...
namespace detail {
template<typename T> class FftIdx {
public:
    MF_CONSTEXPR FftIdx(T n) MF_NOEXCEPT: N(n), forward(T(0)), reverse(T(0)) {}
...
void fft_radix2(Complex<float_t> *z, size_t N) {
    /* поворотные коэффициенты */
    Complex<float_t> *ww = new Complex<float_t>[N / 2 ? N / 2 : 1];
    for(size_t i = 0; i < N / 2; ++i) {
        ww[i] = Complex<float_t>::polar(ONE, -TWO_PI * float_t(i) / float_t(N));
    }

    /* перестановка элементов */
    for(FftIdx<size_t> fft_idx(N); fft_idx != N; ++fft_idx) {
        if(fft_idx.i() < fft_idx.r()) {
            std::swap(z[fft_idx.i()], z[fft_idx.r()]);
        }
    }
...
void czt(const Complex<float_t> *in,
         size_t N,
         Complex<float_t> *out,
         size_t M,
         const Complex<float_t> &w,
         const Complex<float_t> &a) {
    const size_t fft_size = clp2(N + M - 1);

    Complex<float_t> *zz = new Complex<float_t>[fft_size];
    for(size_t k = 0; k < fft_size; ++k) {
        if(k < N) {
            zz[k] = w.pow(HALF * sqr(k)) / a.pow(k) * in[k];
        } else {
    ...
    Complex<float_t> *w2 = new Complex<float_t>[fft_size];
    for(size_t k = 0; k < fft_size; ++k) {
        if(k < N + M - 1) {
            const int kshift = k - (N - 1);
            w2[k] = w.pow(-HALF * sqr(kshift));
...
MF_CONSTEXPR float_t scaler(size_t N, float_t i) MF_NOEXCEPT {
    return (i - float_t(N) / TWO + HALF) / float_t(N);
}
} // namespace detail
...
//                              B-spline windows                              //
////////////////////////////////////////////////////////////////////////////////
template<typename DataType> void rect(DataType *win, size_t N) MF_NOEXCEPT {
    for(size_t n = 0; n != N; ++n) {
        win[n] = ONE;
    }
}
template<typename DataType> void bartlett(DataType *win, size_t N) MF_NOEXCEPT {
    const DataType subtractor = (float_t(N) - ONE) / TWO;
    const DataType divider = (float_t(N) - ONE) / TWO;
    for(size_t n = 0; n != N; ++n) {
        win[n] = ONE - abs((float_t(n) - subtractor) / divider);
    }
}
template<typename DataType> void triang(DataType *win, size_t N) MF_NOEXCEPT {
    const DataType subtractor = (float_t(N) - ONE) / TWO;
    const DataType divider = (float_t(N) + N % 2) / TWO;
    for(size_t n = 0; n != N; ++n) {
        win[n] = ONE - abs((float_t(n) - subtractor) / divider);
    }
}
template<typename DataType> void parzen(DataType *win, size_t N) MF_NOEXCEPT {
    for(size_t n = 0; n != N; ++n) {
        DataType x = abs(TWO * float_t(n) - (float_t(N) - ONE)) / float_t(N);
        DataType y = ONE - x;
...
//                                                 Cosine-sum windows                                                 //
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template<typename DataType> void cosine(DataType *win, size_t N) MF_NOEXCEPT {
    const float_t factor = PI / float_t(N);
    for(size_t n = 0; n != N; ++n) {
        win[n] = sin(factor * (float_t(n) + HALF));
    }
}
template<typename DataType> void bohman(DataType *win, size_t N) MF_NOEXCEPT {
    for(size_t n = 0; n != N; ++n) {
        const float_t factor = abs(TWO * float_t(n) / (float_t(N) - ONE) - ONE);
        win[n] = (ONE - factor) * cos(PI * factor) + INV_PI * sin(PI * factor);
    }
...
void cosine_sum(DataType *win, size_t N, const float_t (&a)[K], bool scale = true) MF_NOEXCEPT {
    const float_t factor = TWO_PI / (float_t(N) - ONE);
    float_t scaler = 0;
    for(size_t n = 0; n != K; ++n) {
        scaler += a[n];
    }
...
template<typename DataType> void general_hamming(DataType *win, size_t N, float_t alpha) {
    static float_t a[2];
    a[0] = alpha;
    a[1] = ONE - alpha;
    cosine_sum(win, N, a);
}
template<typename DataType> void hamming(DataType *win, size_t N) MF_NOEXCEPT {
    general_hamming(win, N, MF_FLOAT_MAX_C(0.54));
}
template<typename DataType> void hann(DataType *win, size_t N) MF_NOEXCEPT {
    general_hamming(win, N, HALF);
}
template<typename DataType> void blackman_generic(DataType *win, size_t N, float_t alpha) MF_NOEXCEPT {
    static float_t a[3];
    a[0] = (ONE - alpha) / TWO;
    a[1] = HALF;
    a[2] = alpha / TWO;
...
template<typename DataType> void blackman(DataType *win, size_t N) MF_NOEXCEPT {
    blackman_generic(win, N, MF_FLOAT_MAX_C(0.16));
}
template<typename DataType> void nuttall3(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(0.375),
        MF_FLOAT_MAX_C(0.5),
        MF_FLOAT_MAX_C(0.125),
...
template<typename DataType> void nuttall3a(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(0.40897),
        MF_FLOAT_MAX_C(0.5),
        MF_FLOAT_MAX_C(0.09103),
...
template<typename DataType> void nuttall3b(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(0.4243801),
        MF_FLOAT_MAX_C(0.4973406),
        MF_FLOAT_MAX_C(0.0782793),
...
template<typename DataType> void nuttall4(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(0.3125),
        MF_FLOAT_MAX_C(0.46875),
        MF_FLOAT_MAX_C(0.1875),
...
template<typename DataType> void nuttall4a(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(0.338946),
        MF_FLOAT_MAX_C(0.481973),
        MF_FLOAT_MAX_C(0.161054),
...
template<typename DataType> void nuttall4b(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(0.355768),
        MF_FLOAT_MAX_C(0.487396),
        MF_FLOAT_MAX_C(0.144232),
...
template<typename DataType> void nuttall4c(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(0.3635819),
        MF_FLOAT_MAX_C(0.4891775),
        MF_FLOAT_MAX_C(0.1365995),
...
template<typename DataType> void blackmanharris(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(0.35875),
        MF_FLOAT_MAX_C(0.48829),
        MF_FLOAT_MAX_C(0.14128),
...
//                              Flat-top windows                              //
////////////////////////////////////////////////////////////////////////////////
template<typename DataType> void flattop(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(0.21557895),
        MF_FLOAT_MAX_C(0.41663158),
        MF_FLOAT_MAX_C(0.277263158),
    ...
    cosine_sum(win, N, a, false);
}
/**
 * @brief Fast decaying 3-term flat top window
 */
template<typename DataType> void sft3f(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(0.26526),
        MF_FLOAT_MAX_C(0.5),
        MF_FLOAT_MAX_C(0.23474),
 ...
 * @brief Fast decaying 4-term flat top window
 */
template<typename DataType> void sft4f(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(0.21706),
        MF_FLOAT_MAX_C(0.42103),
        MF_FLOAT_MAX_C(0.28294),
 ...
 * @brief Fast decaying 5-term flat top window
 */
template<typename DataType> void sft5f(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(0.1881),
        MF_FLOAT_MAX_C(0.36923),
        MF_FLOAT_MAX_C(0.28702),
 ...
 * @brief Minimum sidelobe 3-term flat top window
 */
template<typename DataType> void sft3m(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(0.28235),
        MF_FLOAT_MAX_C(0.52105),
        MF_FLOAT_MAX_C(0.19659),
 ...
 * @brief Minimum sidelobe 4-term flat top window
 */
template<typename DataType> void sft4m(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(0.241906),
        MF_FLOAT_MAX_C(0.460841),
        MF_FLOAT_MAX_C(0.255381),
 ...
 * @brief Minimum sidelobe 5-term flat top window
 */
template<typename DataType> void sft5m(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(0.209671),
        MF_FLOAT_MAX_C(0.407331),
        MF_FLOAT_MAX_C(0.281225),
 ...
 * @brief National Instruments flat-top window
 */
template<typename DataType> void ftni(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(0.2810639),
        MF_FLOAT_MAX_C(0.5208972),
        MF_FLOAT_MAX_C(0.1980399),
 ...
 * @brief Old HP flat-top window
 */
template<typename DataType> void fthp(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(1.0),
        MF_FLOAT_MAX_C(1.912510941),
        MF_FLOAT_MAX_C(1.079173272),
 ...
 * @brief Stanford Research flat-top window
 */
template<typename DataType> void ftsrs(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(1.0),
        MF_FLOAT_MAX_C(1.93),
        MF_FLOAT_MAX_C(1.29),
...
template<typename DataType> void hft70(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(1.0),
        MF_FLOAT_MAX_C(1.90796),
        MF_FLOAT_MAX_C(1.07349),
...
template<typename DataType> void hft95(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(1.0),
        MF_FLOAT_MAX_C(1.9383379),
        MF_FLOAT_MAX_C(1.3045202),
...
template<typename DataType> void hft90d(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(1.0),
        MF_FLOAT_MAX_C(1.942604),
        MF_FLOAT_MAX_C(1.340318),
...
template<typename DataType> void hft116d(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(1.0),
        MF_FLOAT_MAX_C(1.9575375),
        MF_FLOAT_MAX_C(1.4780705),
...
template<typename DataType> void hft144d(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(1.0),
        MF_FLOAT_MAX_C(1.96760033),
        MF_FLOAT_MAX_C(1.57983607),
...
template<typename DataType> void hft169d(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(1.0),
        MF_FLOAT_MAX_C(1.97441843),
        MF_FLOAT_MAX_C(1.65409889),
...
template<typename DataType> void hft196d(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(1.0),
        MF_FLOAT_MAX_C(1.979280420),
        MF_FLOAT_MAX_C(1.710288951),
...
template<typename DataType> void hft223d(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(1.0),
        MF_FLOAT_MAX_C(1.98298997309),
        MF_FLOAT_MAX_C(1.75556083063),
...
template<typename DataType> void hft248d(DataType *win, size_t N) MF_NOEXCEPT {
    static const float_t a[] = {
        MF_FLOAT_MAX_C(1.0),
        MF_FLOAT_MAX_C(1.985844164102),
        MF_FLOAT_MAX_C(1.791176438506),
...
//                             Adjustable windows                             //
////////////////////////////////////////////////////////////////////////////////
template<typename DataType> void gaussian(DataType *win, size_t N, float_t sigma) MF_NOEXCEPT {
    const float_t subtractor = (float_t(N) - ONE) / TWO;
    for(size_t n = 0; n != N; ++n) {
        const float_t x = float_t(n) - subtractor;
        win[n] = exp(-HALF * sqr(x) / sqr(sigma));
    }
}
template<typename DataType> void tukey(DataType *win, size_t N, float_t alpha) MF_NOEXCEPT {
    if(alpha <= 0.0) {
        rect(win, N);
        return;
    } else if(alpha >= ONE) {
...
template<typename DataType> void kaiser(DataType *win, size_t N, float_t beta) MF_NOEXCEPT {
    for(size_t n = 0; n != N; ++n) {
        const float_t factor = TWO * float_t(n) / (float_t(N) - ONE) - ONE;
        win[n] = bessel::i0(beta * sqrt(ONE - sqr(factor))) / bessel::i0(beta);
    }
}
template<typename DataType> void kaiser_bessel_derived(DataType *win, size_t N, float_t beta) MF_NOEXCEPT {
    kaiser(win, N / 2 + 1, beta);
...
template<typename DataType> void chebyshev(DataType *win, size_t N, float_t alpha) MF_NOEXCEPT {
    const size_t order = N - 1;
    const float_t amp = pow(float_t(10), abs(alpha) / float_t(20));
    const float_t beta = cosh(acosh(amp) / order);

    Complex<float_t> *W = new Complex<float_t>[N];
    if(N % 2) {
        for(size_t n = 0; n < N; ++n) {
            const float_t x = beta * cos(PI * float_t(n) / float_t(N));
            if(x > ONE) {
...
void taylor(DataType *win, size_t N, float_t sll = 30, size_t nbar = 4, bool norm = true) MF_NOEXCEPT {
    const size_t MAX_NBAR = (N / 2 < 4) ? 4 : N / 2;
    ...
    float_t *Fm = new float_t[MAX_NBAR - 1];
    for(size_t mi = 0; mi < nbar - 1; ++mi) {
        float_t numer = (mi % 2) ? -1 : 1;
        for(size_t i = 0; i < nbar - 1; ++i) {
            numer *= (ONE - sqr(mi + ONE) / (s2 * (sqr(A) + sqr(i + HALF))));
...
template<typename DataType> void poisson(DataType *win, size_t N, float_t tau) MF_NOEXCEPT {
    const float_t subtractor = (float_t(N) - ONE) / TWO;
    for(size_t n = 0; n != N; ++n) {
        win[n] = exp(-abs(float_t(n) - subtractor) / tau);
    }
...
//                               Hybrid windows                               //
////////////////////////////////////////////////////////////////////////////////
template<typename DataType> void barthann(DataType *win, size_t N) MF_NOEXCEPT {
    for(size_t n = 0; n != N; ++n) {
        const float_t factor = abs(float_t(n) / (float_t(N) - ONE) - HALF);
        win[n] = MF_FLOAT_MAX_C(0.62) - MF_FLOAT_MAX_C(0.48) * factor + MF_FLOAT_MAX_C(0.38) * cos(TWO_PI * factor);
    }
...
//                               Other windows                                //
////////////////////////////////////////////////////////////////////////////////
template<typename DataType> void lanczos(DataType *win, size_t N) MF_NOEXCEPT {
    const float_t factor = TWO / (float_t(N) - ONE);
    for(size_t n = 0; n != N; ++n) {
        win[n] = sinc(factor * float_t(n) - ONE);
    }
}
